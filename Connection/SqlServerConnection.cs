...
{
    public class SqlServerConnection : MyConnection
    {
        public SqlServerConnection(IDbConnection conn)
        {
            this.conn = conn;
        }
        ...
        {
            return ExecuteScalar<T>("SELECT @@IDENTITY");
        }
        ...
        public override IEnumerable<T> GetByIdsWhichField<T>(object ids, string whichField, string returnFields = null)
        {
            ...
            if (string.IsNullOrEmpty(returnFields))
                returnFields = sqls.AllFields;
            string sql = string.Format("SELECT {0} FROM [{1}] WITH(NOLOCK) WHERE [{2}] IN @ids", returnFields, sqls.TableName, whichField);
            return Query<T>(sql, dpar);
        }
